...
class ActorNoise:
    ...
    def ou_reset(self):
        self.x_prev = zeros_like(tensor([self.mu]))
...
class Actor(nn.Module):
    ...
    def noisy_sample(self,s,min_action,max_action):

        
        action,log_prob = self.sample(s)
        
        noisy_action = self.noise.normal_noise(action)
        clipped_noisy_action = torch.clamp(noisy_action, min_action, max_action)
        
        return clipped_noisy_action,log_prob
    
    def ou_noisy_sample(self,s,min_action,max_action):
